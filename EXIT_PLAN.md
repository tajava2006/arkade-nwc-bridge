...
- ✅ **stuck 패키지 fee 재부스트** (`feat/exit-boost`, 2026-07-11): 멤풀 스파이크로 1P1C 패키지가 밀릴 때 CPFP 자식을 RBF로 교체 + 패키지 재제출(`src/exit/boost.ts` + `broadcasts.ts`, 마이그레이션 v15). SDK엔 재범프 경로 없음(Session은 멤풀 tx에 WAIT 무한). 확정 설계: **부모(pre-signed·zero-fee) 불변 → 항상 자식만 교체**, fee = max(다음블록 rate × 패키지 vB, RBF floor[옛fee + incrementalRelay×새vB]), **confirmed-only fuel + stuck 자식 prevout 회수**(TRUC unconfirmed-부모-1개 제한 + 교체대상 output 지출 방지), 에러 표면화(bumpP2A는 삼킴). sweep도 동일 RBF. UI: 멤풀 스텝에 `패키지 rate vs 다음블록 + N블록 대기` 표시, **버튼 활성 = rate 미달일 때만**(대기시간은 참고), **fee picker 없음**(다음블록 프리셋 1개), **auto-boost 없음**(운영자 발동만).
  **✅ regtest 드릴 통과 (2026-07-12, `regtest-e2e/boost-drill.sh` — blocks 모드라 stuck이 구조적으로 재현됨, 완전 자동)**: 실bitcoind가 자식 RBF(278→433 sats, 부모 불변) + sweep RBF(148→287) 둘 다 수락, prevout 회수 경로(연료 코인 1개 강제)로 완주. 드릴이 잡아낸 실갭 3개 → `fix/boost-anchor-spender-fallback`로 수정: ① **arkade-mempool계 esplora는 `/outspends`에서 spender txid 생략**(mainnet mempool.arkade.sh도 동일) → 앵커 자식 식별 폴백 추가(`EsploraReader.spenderFromAddress` — 우리 자식은 잔돈이 연료 주소로 가므로 address-txs에서 앵커 outpoint 지출 tx를 스캔), ② **blind-boost 가드**: 부모가 멤풀에 있는데 자식 fee를 못 읽으면(esplora 인덱스 랙) floor 없는 재조립 = 같은 fee·같은 txid → 노드가 dedup하는 조용한 no-op — 시끄럽게 거부하고 재시도 유도, ③ **`/txs/package`는 submitpackage 거부를 200에 실을 수 있음**(SDK는 상태코드만 봄) → 브로드캐스트 후 멤풀 등장 양성 확인(`confirmInMempool`). sweep boost도 같은 가드/확인 적용.
